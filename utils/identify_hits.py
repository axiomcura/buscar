...
def identify_compound_hit(
    ...
    paired_scores_df = (
        distance_df.lazy()
        # Sort by treatment cluster, then by on_dist and off_dist
        .sort(["treatment", "exp_cluster", "on_dist", "off_dist"])
        # For each treatment cluster, take the first (best matching) ref_cluster
        .group_by(["treatment", "exp_cluster"])
        .agg([pl.all().first()])
        # Rename columns to match what identify_compound_hit expects
        .rename(
            {
                "exp_cluster": "treatment_cluster_id",
                "on_dist": "on_score",
                "off_dist": "off_score",
                "exp_cluster_ratio": "ratio",
            }
        )
        .collect()
    )
